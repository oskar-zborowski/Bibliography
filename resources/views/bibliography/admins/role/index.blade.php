...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><a href="{{ route("bibliography.admins.index") }}">{{ trans('admiko.admins_title') }}</a></li>
...
@section('backBtn')
    <a href="{{route('bibliography.admins.index')}}"><i class="fas fa-angle-left"></i> {{ trans('admiko.page_back_btn') }}</a>
@endsection
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="tableBox">
                    ...
                    <table class="table" data-dom="tr" data-page-length='-1'>
                        ...
                        @foreach($tableData as $data)
                            <tr>
                                ...
                                <td class=" nowrap"><a href="{{route('bibliography.admin_roles.edit',$data->id)}}">{{{$data->title}}}</a></td>
                                <td class="w-5 no-sort">
                                    @if($data->id != 1)
                                        <a href="{{route('bibliography.admin_roles.edit',$data->id)}}"><i class="fas fa-edit fa-fw"></i></a>
                                    @endIf
                                </td>
                                <td class="w-5 no-sort">
                                    @if($data->id != 1)
                                        <a href="{{route('bibliography.admin_roles.destroy',$data->id)}}" data-id="{{$data->id}}" class="admiko_deleteConfirm" data-bs-toggle="modal" data-bs-target="#deleteConfirm"><i class="fas fa-trash fa-fw"></i></a>
                                    @endIf
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="row">
                    <div class="col-12 col-sm order-3 order-sm-0 pt-3">
                        <a href="{{route('bibliography.admin_roles.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{ trans('admiko.table_add') }}</a>
                    </div>
        ...
        <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
            <div class="modal-dialog">
                <form method="post" action="{{route("bibliography.admin_roles.delete")}}">
                    @method('DELETE')
                    @csrf
