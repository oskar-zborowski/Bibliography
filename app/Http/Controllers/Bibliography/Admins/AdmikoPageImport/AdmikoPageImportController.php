...
namespace App\Http\Controllers\Bibliography\Admins\AdmikoPageImport;
...
class AdmikoPageImportController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("bibliography.admins.admiko_page_import.index")->with(compact('admiko_data', "tableData", "admikoUpdateInfo", "errors"));
    }
    ...
    public function import(Request $request)
    {
        ...
        if ($request->admikoUpdate == 1) {
            ...
            if (isset($dataApi->error) && $dataApi->error != '') {
                return redirect(route("bibliography.admiko_page_import"))->with('error', $dataApi->error);
            }
...
//                    return redirect(route("dashboard.admiko_page_import"))->with('error', $dataApi->error);
    ...
    public function backupAndSave($file, $code, $backup_folder)
    {
        if (config('filesystems.disks.admiko_api_import')) {
            ...
            return redirect(route("bibliography.admiko_page_import"))->with('error', trans('admiko.admiko_api_import_missing'));
        }
    }
