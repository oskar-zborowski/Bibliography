...
class AdminGlobalSearchController extends Controller
{
    ...
    public function global_search(Request $request)
    {
        ...
        foreach ($this->mergeForGlobalSearch() as $data){
            $modelClass = 'App\Models\Bibliography\\' . $data['model'];
            ...
            if(count($results) > 0){
                foreach ($results as $result) {
                    ...
                    $parsedData['url'] = route('bibliography.'.$data['route_id'].'.edit',$ids);
                    $items[] = $parsedData;
                }
                array_pop($ids);
                $searchableData[] = ['name'=>$data['name'],'index_url'=>route('bibliography.'.$data['route_id'].'.index',$ids),'items'=>$items];
            }
        }
    ...
    public function getUrlId($model,$results){
        ...
        $model = 'App\Models\Bibliography\\'.$model;
    ...
    public function getParentUrlId($modelPath,$results){
        if (property_exists(app($modelPath), 'admikoGlobalSearchParent')) {
            ...
            $child_parent_id = $modelPath::$admikoGlobalSearchParent['child_parent_id'];
             $modelClass = 'App\Models\Bibliography\\' . $parent_model;
